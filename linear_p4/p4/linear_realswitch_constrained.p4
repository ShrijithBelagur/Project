...
struct metadata_t {
    ...
    bit<32> idx0; bit<32> tag;
    bit<1> valid0; bit<32> stored_tag0; bit<32> stored_last_ts;
...
control MyIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    action set_output_from_prediction() {
        if (meta.predicted_label == 8w0) { standard_metadata.egress_spec = 9w1; }
        else if (meta.predicted_label == 8w1) { standard_metadata.egress_spec = 9w2; }
        else if (meta.predicted_label == 8w2) { standard_metadata.egress_spec = 9w3; }
    ...
    action classify_linear_nomul() {
        /* Exact shift/add decomposition of the current trained model. */
        meta.score0 = 32s7154;
        meta.score0 = meta.score0 + meta.f1;
        meta.score0 = meta.score0 + (meta.f3 << 6) + (meta.f3 << 5) + (meta.f3 << 2);
        meta.score0 = meta.score0 - (meta.f4 << 10) - (meta.f4 << 6) - (meta.f4 << 5) - (meta.f4 << 1) - meta.f4;
        meta.score0 = meta.score0 - meta.f5;
        meta.score0 = meta.score0 - (meta.f6 << 4) - (meta.f6 << 3) - meta.f6;
        meta.score0 = meta.score0 + (meta.f9 << 1);

        meta.score1 = -32s4433;
        meta.score1 = meta.score1 - meta.f1;
        meta.score1 = meta.score1 - (meta.f3 << 6) - (meta.f3 << 5) - (meta.f3 << 4) - meta.f3;
        meta.score1 = meta.score1 + (meta.f4 << 9) + (meta.f4 << 6) + (meta.f4 << 5) + (meta.f4 << 4) + (meta.f4 << 3) + (meta.f4 << 1) + meta.f4;
        meta.score1 = meta.score1 - meta.f5;
        meta.score1 = meta.score1 + (meta.f6 << 4) + (meta.f6 << 3) + (meta.f6 << 1) + meta.f6;
        meta.score1 = meta.score1 + meta.f7;
        meta.score1 = meta.score1 - meta.f9;

        meta.score2 = -32s10913;
        meta.score2 = meta.score2 + (meta.f3 << 3) + (meta.f3 << 2) + meta.f3;
        meta.score2 = meta.score2 + (meta.f4 << 8) + (meta.f4 << 7) + (meta.f4 << 6) + (meta.f4 << 5) + (meta.f4 << 3);
        meta.score2 = meta.score2 + (meta.f5 << 1);
        meta.score2 = meta.score2 - (meta.f6 << 1) - meta.f6;
        meta.score2 = meta.score2 - meta.f9;
    ...
    apply {
        if (hdr.ipv4.isValid() && hdr.tcp.isValid()) {
            ...
            prepare_linear_features(); classify_linear_nomul(); set_output_from_prediction();
        } else { drop(); }
    }
}
