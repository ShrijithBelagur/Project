...
// BMv2-relaxed state profile:
// - Same manual 10x8x3 runtime-loaded BNN inference core as the constrained P4.
// - Two independent 65536-slot tagged flow-state banks, for 131072 total slots.
// - 32-bit fingerprint tags and older-slot overwrite reduce state-loss artifacts.
// - If both candidate banks hold other flows, overwrite the older slot.
// - Uses the same runtime-loaded 10x8x3 model as the constrained P4 variant.

const bit<16> TYPE_IPV4 = 0x0800;
const bit<8> PROTO_TCP = 6;
const bit<32> RELAXED_BANK_SLOTS = 32w65536;
const bit<32> RELAXED_TAG_SPACE = 32w4294967295;
...
register<bit<2>>(80) nn_w1_reg; register<int<16>>(8) nn_th1_reg; register<int<16>>(8) nn_b1_reg;
register<bit<2>>(24) nn_w2_reg; register<int<16>>(3) nn_b2_reg;
...
control MyIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    action compute_hashes() {
        hash(meta.idx0, HashAlgorithm.crc16, 32w0, { hdr.ipv4.protocol, meta.ip_a, meta.port_a, meta.ip_b, meta.port_b }, RELAXED_BANK_SLOTS);
        hash(meta.idx1, HashAlgorithm.crc32, 32w0, { hdr.ipv4.protocol, meta.ip_a, meta.port_a, meta.ip_b, meta.port_b }, RELAXED_BANK_SLOTS);
        hash(meta.tag, HashAlgorithm.crc32, 32w0, { hdr.ipv4.protocol, meta.ip_a, meta.port_a, meta.ip_b, meta.port_b }, RELAXED_TAG_SPACE);
    }
